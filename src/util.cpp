...
const char * const BITCOIN_CONF_FILENAME = "akula.conf";
const char * const BITCOIN_PID_FILENAME = "akulad.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AkulaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AkulaCore
    // Mac: ~/Library/Application Support/AkulaCore
    // Unix: ~/.akulacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AkulaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AkulaCore";
#else
    // Unix
    return pathRet / ".akulacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty akula.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override akula.conf
